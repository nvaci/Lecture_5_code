...
```{r}
MWmod1<-lmer(mwb~(1|LANAME), data=train) #random effect of Local area
MWmod2<-lmer(mwb~(1|LANAME)+(1|Ethnicg), data=train) #crossed random effects of local area and ethnicity
MWmod3<-lmer(mwb~(1|REGION/LANAME)+(1|Ethnicg), data=train) #nested random effect of local area that is nested in region and crossed with ethnicity
...
```{r}
MWmod2a<-lmer(mwb~Total+(1|LANAME)+(1|Ethnicg), data=train) #main effect of total
...
```{r}
MWmod2b<-lmer(mwb~Total+male+(1|LANAME)+(1|Ethnicg), data=train) #main effect of total and main effect of sex
MWmod2c<-lmer(mwb~Total*male+(1|LANAME)+(1|Ethnicg), data=train) #interaction between total and sex
...
```{r}
MWmod3c<-lmer(mwb~Total*male+(1+Total|LANAME)+(1|Ethnicg), data=train) #random slopes for total predictor for Local area and intercept for local area 
```

```{r}
MWmod3c<-lmer(mwb~Total*male+(1|LANAME:male)+(1|Ethnicg), data=train) #random intercept for unique combination between local area and sex 
anova(MWmod2c, MWmod3c) #comparison of the models
```

```{r}
MWmod3c<-lmer(mwb~Total*male+(1|LANAME)+(1|Ethnicg:male), data=train) #random intercepts for unique combination between Ethnicity and sex
...
```{r, fig.width=12, fig.height=5, fig.align='center'}
train$predicted=predict(MWmod3c) #prediction values from model to our train dataset
par(mfrow=c(1,1), bty='n',mar = c(5, 4, .1, .1), cex=1.1,pch=16)
plot(train$predicted, train$mwb) #plot predicted versus observed data points
```
...
```{r}
MWmod3d<-lmer(mwb~poly(Total, degree=2, raw=T)*male+(1|LANAME)+(1|Ethnicg:male), data=train) #poly adds polynomial transformations, degree = 2 adds second order polynomial (quadratic function), while raw = T indicates that we do not want to use orthogonal polynomials
```

Means squared error of prediction 2: 
